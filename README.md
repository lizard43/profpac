...
| P
rogram storage | Nine 8 KB `pps` ROMs in fixed and banked windows |
...
flowchart TD
    ...
    Main --> Attrac
t["Attract, credits, and start control"]
...
is documented in [question_round.md](docs/ques
tion_round.md).
...
T
he EPROM board provides forty 16 KB question-bank positions. The production
...
| `IX` | Downward-gro
wing threaded return stack, initialized to `$EF50` |
...
- [terse_vocabulary.md](docs/terse_vocabulary.md) catalogs re
sident words, stack
...
flowchart TD
    ...
    Actions --> Resolve["Co
llect response and update state"]
```
...
- [question_roms.md](docs/question_roms.md) specifies the ba
nk directory, tier
...
operator-settings menus. Its ROM test
s cover the populated program and question
...
| `tools/analyze_game_progression.py` | Configuration-1 application closure valida
tion |
...
The build performs the following c
hecks:
