...
![](images/professor-pac-man-31925.jpg)
...
and initials entry. The decoded configuration-1 graph is documented in
...
`pps1`, `pps2`, and `pps9` remain fixed in the CPU address space. Port `$F3`
selects the two banked read windows. Configuration 0 maps `pps3` and `pps4` as
question-round support; configuration 1 maps `pps5` and `pps6` into the lower
window and `pps7` and `pps8` into the upper window for the main application.
Writes to `$4000-$7FFF` always reach 16-color screen RAM, even when reads from
that range return program or question ROM.

The 640 KB EPROM board provides forty 16 KB question-bank positions. The
production set populates banks 0 through 13 with `ppq1`-`ppq14`; banks 14
through 39 are empty. A question selector maps one complete PPQ module into
`$4000-$7FFF` while retaining the supporting program configuration in
`$8000-$BFFF`.

![Professor Pac-Man question EPROM map](images/professor-pacman-question-rom-map.png)

The complete memory, I/O, banking, video, and control specification is in
...
output enables, polarities, and Boolean equations are documented in
...
The initializers, action lists, and presentation tasks are compiled
direct-threaded TERSE routines. The following section defines the runtime that
executes them.
