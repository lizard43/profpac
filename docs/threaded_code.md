...
## Self-test/operator thread

Cold startup reads the active-low cabinet test input. When asserted, it selects
the threaded instruction pointer at `$6DD2`; the body is also callable through
`INITIAL_THREAD_WORD` at `$6DD1`. This path:
...
6. enters the self-test/operator dispatcher at `$6DA7`.

The first-initialization path clears state, initializes the diagnostic
structures, stores the initial value at `$FEAA`, and enters the operator menu.
The established-state path may resume the continuous-test sequence at `$6B7E`.

## Normal-play thread

thread is:
With the test switch released, startup initializes `BC` to `$DEBE`. Its
complete outer thread is:
thread is:
...
NORMAL_PLAY_THREAD:
        ...
        dw      $00F3
        dw      _OUTP
        dw      CFG1_MAIN_APPLICATION_XT ; MAIN_APPLICATION
        dw      _RETURN
```

The thread selects program-bank configuration 1 through port `$F3`, invokes
`MAIN_APPLICATION` at `$BE66`, and returns. Its attract, start, turn,
question-handoff, game-over, and high-score graph is documented in
[game_progression.md](game_progression.md).

## Self-test call graph

The test-switch application has been resolved from its counted menu strings,
