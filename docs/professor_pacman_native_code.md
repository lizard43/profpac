...
2. Program the initial video registers and initialize IX to `$EF50` and SP to
   `$F000`.
3. Probe RAM from `$E280`, test the service input, and select program-bank
...
6. Select the normal thread at `$6DD2`, or the service thread at `$DEBE`, and
...
Every immediate write to port `$F3` is in `pps1`:
...
Values `$80-$A7` use the same latch to select one of forty 16 KB question-EPROM
banks at `$4000-$7FFF`. Their callers are TERSE words and are mapped during the
threaded-code pass.
