...
# zmac 1.3 retains forty characters per symbol. These source prefixes keep the
# family identity readable while leaving room for lifecycle and role suffixes.
FAMILY_SYMBOL_PREFIXES = {
    "MIRROR_FLOCK": "MIRROR_FLOCK",
    "FILL_IN_THE_BLANK": "FILL_BLANK",
    "DIALED_NUMBER_MEMORY": "DIALED_NUMBER",
    "COLORED_CIRCLE_COUNT": "CIRCLE_COUNT",
    "LINE_INTERSECTION_COUNT": "LINE_COUNT",
    "FIGURE_MIRROR_MATCH": "FIGURE_MIRROR",
    "PICTURE_COUNT_MEMORY": "PICTURE_COUNT",
    "IMAGE_MIRROR_MATCH": "IMAGE_MIRROR",
    "COMPLETE_FIGURE_A": "COMPLETE_FIGURE_A",
    "IMAGE_MIRROR_MATCH_A": "IMAGE_MIRROR_A",
    "IMAGE_MIRROR_MATCH_B": "IMAGE_MIRROR_B",
    "JUGGLER_MEMORY": "JUGGLER_MEMORY",
    "COMPLETE_FIGURE_B": "COMPLETE_FIGURE_B",
    "LEFT_RIGHT_TURNS": "LEFT_RIGHT_TURNS",
    "VISUAL_ANALOGY": "VISUAL_ANALOGY",
    "OPERATION_SEQUENCE": "OPERATION_SEQUENCE",
    "NUMBER_TRANSFORMATION": "NUMBER_TRANSFORM",
    "NUMBER_PROGRESSION": "NUMBER_PROGRESSION",
    "HAND_PLACEMENT_MEMORY": "HAND_MEMORY",
    "FIGURE_SERIES_MEMORY": "FIGURE_MEMORY",
    "BOX_MEMBERSHIP": "BOX_MEMBERSHIP",
    "IMAGE_MIRROR_PAIR": "IMAGE_MIRROR_PAIR",
    "OBJECT_COUNT_MEMORY": "OBJECT_COUNT",
    "VISUAL_PROGRESSION": "VISUAL_PROGRESSION",
    "TABLE_SETTING_MEMORY": "TABLE_MEMORY",
    "COMPLETE_FIGURE_C": "COMPLETE_FIGURE_C",
    "COMPLETE_FIGURE_D": "COMPLETE_FIGURE_D",
    "COMPLETE_FIGURE_E": "COMPLETE_FIGURE_E",
    "UNFOLDED_DIE": "UNFOLDED_DIE",
    "CITY_OBJECT_COUNT": "CITY_OBJECT_COUNT",
    "CITY_SCENE_REASONING": "CITY_REASONING",
    "GROUP_MEMBERSHIP": "GROUP_MEMBERSHIP",
    "CUBE_COUNT": "CUBE_COUNT",
    "ODD_ONE_OUT": "ODD_ONE_OUT",
    "DOG_MIRROR_MATCH": "DOG_MIRROR_MATCH",
    "DEER_MIRROR_MATCH": "DEER_MIRROR_MATCH",
}
...
def family_symbol(family: Family) -> str:
    return f"PPQ{family.bank}_{family_prefix(family.name)}_ACTIONS"


def family_prefix(name: str) -> str:
    return FAMILY_SYMBOL_PREFIXES[name]
...
def render_initializer(initializer: Initializer, symbol: str, name: str) -> str:
    lines = [f"{name}:"]
...
LEGACY_EXECUTION_LABEL = re.compile(
    r"^(?:PPQ\d+_TERSE|QUESTION_INITIALIZER|QUESTION_BUCKET)_[0-9A-F]{4}$"
)
...
def existing_definition_labels(lines: list[str]) -> dict[int, str]:
    ...
    for index, line in enumerate(lines):
        ...
        if match is None or LEGACY_EXECUTION_LABEL.match(match.group(1)):
            continue
...
def labels_by_emitted_address(lines: list[str]) -> dict[int, list[str]]:
    ...
    for line in lines:
        ...
        if pending and address:
            labels.setdefault(int(address.group(1), 16), []).extend(pending)
            pending = []
...
KNOWN_DEFINITION_NAMES = {
    (1, 0x525F): "MIRROR_FLOCK_PREPARE_ANSWER",
    (1, 0x5279): "MIRROR_FLOCK_IMAGE_ACTION_0",
    (1, 0x5284): "MIRROR_FLOCK_IMAGE_ACTION_1",
    (1, 0x528F): "MIRROR_FLOCK_IMAGE_ACTION_2",
}


def definition_role(definition: Definition) -> str:
    tokens = {cell.token for cell in definition.cells}
    if INLINE_CASES & tokens:
        return "VARIANT"
    if 0x1BDF in tokens:
        return "ANIM"
    if any(cell.token in BRANCH | CONDITIONAL_BRANCH and
           cell.operand is not None and cell.operand < cell.address
           for cell in definition.cells):
        return "LOOP"
    if {0x1DF3, 0x1E0C, 0x2CF2} & tokens:
        return "RENDER"
    if 0x1B32 in tokens:
        return "RANDOM"
    if {0x0532, 0x053A, 0x053F, 0x055C, 0x0561} & tokens:
        return "STATE"
    if 0x2302 in tokens:
        return "JOIN"
    return "THREAD"
...
def reachable_definitions(
    start: int, definitions: dict[int, Definition]
) -> set[int]:
    pending = [start]
    reached: set[int] = set()
    while pending:
        address = pending.pop()
        if address in reached or address not in definitions:
            continue
        reached.add(address)
        pending.extend(definitions[address].calls)
    return reached
...
def definition_names(
    ...
    names = {
        address: name
        for (name_bank, address), name in KNOWN_DEFINITION_NAMES.items()
        if name_bank == bank
    }
    definitions = {
        definition.address: definition
    ...
    roots = {
        address
        for family in bank_families
        for address in family.actions
    }
    for family in bank_families:
        for address, stage in zip(family.actions, ACTION_STAGE_NAMES):
            names[address] = f"{family_prefix(family.name)}_{stage}"

    ownership: dict[int, set[tuple[str, int]]] = {
        address: set() for address in definitions
    }
    for family in bank_families:
        for stage, root in enumerate(family.actions):
            for address in reachable_definitions(root, definitions):
                ownership[address].add((family.name, stage))

    generated: dict[int, str] = {}
    for address, definition in definitions.items():
        if address in roots or address in names:
            continue
        owners = ownership[address]
        families = {family for family, _ in owners}
        stages = {stage for _, stage in owners}
        prefix = (
            family_prefix(next(iter(families)))
            if len(families) == 1 else f"PPQ{bank}_SHARED"
        )
        if stages == {0}:
            stage_name = "SETUP"
        elif stages == {1}:
            stage_name = "CORRECT"
        elif stages and stages <= {2, 3}:
            stage_name = "DIST"
        elif stages and stages <= {1, 2, 3}:
            stage_name = "ANSWER"
        else:
            stage_name = "PRESENT"
        generated[address] = f"{prefix}_{stage_name}_{definition_role(definition)}"

    groups: dict[str, list[int]] = {}
    for address, base in generated.items():
        groups.setdefault(base, []).append(address)
    for base, addresses in groups.items():
        for ordinal, address in enumerate(sorted(addresses), 1):
            names[address] = base if len(addresses) == 1 else f"{base}_{ordinal}"
    return names


def initializer_base_name(family: Family, initializer: Initializer) -> str:
    cells = initializer.cells
    tokens = [cell.token for cell in cells]
    variant_store = next(
        (
            index for index, cell in enumerate(cells)
            if cell.token == 0x8B2B
            or (cell.token in {0x0532, 0x053A, 0x053F}
                and index and cells[index - 1].operand == 0xF6E9)
        ),
        None,
    )
    qualifier = ""
    if variant_store is not None:
        prior = cells[:variant_store]
        random_indices = [index for index, cell in enumerate(prior) if cell.token == 0x1B32]
        if random_indices:
            random_index = random_indices[-1]
            limit = prior[random_index - 1].operand
            offset = 0
            if random_index + 2 < len(prior) and prior[random_index + 2].token == 0x015A:
                offset = prior[random_index + 1].operand or 0
            if limit is not None:
                qualifier = f"_VARIANTS_{offset}_TO_{offset + limit - 1}"
        else:
            value = None
            if prior:
                if prior[-1].token == 0x012A:
                    value = 0
                elif prior[-1].token == 0x0130:
                    value = 1
                elif prior[-1].token == 0x0112:
                    value = prior[-1].operand
            qualifier = "_FIXED_VARIANT" if value is None else f"_VARIANT_{value}"
    tiers = "_".join(str(tier) for tier in initializer.tiers)
    qualifier = qualifier.replace("_VARIANTS_", "_V").replace("_TO_", "_")
    qualifier = qualifier.replace("_FIXED_VARIANT", "_VFIXED").replace("_VARIANT_", "_V")
    return f"{family_prefix(family.name)}{qualifier}_T{tiers}_INIT"


def initializer_names(bank_families: list[Family]) -> dict[int, str]:
    bases = {
        initializer.address: initializer_base_name(family, initializer)
        for family in bank_families
        for initializer in family.initializers
    }
    groups: dict[str, list[int]] = {}
    for address, base in bases.items():
        groups.setdefault(base, []).append(address)
    return {
        address: base if len(groups[base]) == 1 else f"{base}_{ordinal}"
        for base, addresses in groups.items()
        for ordinal, address in enumerate(sorted(addresses), 1)
    }


def bucket_names(bank) -> dict[int, str]:
    tiers: dict[int, list[int]] = {}
    for tier, bucket in enumerate(bank.buckets):
        tiers.setdefault(bucket.address, []).append(tier)
    return {
        address: "QUESTION_TIER_" + "_".join(str(tier) for tier in values) + "_BUCKET"
        for address, values in tiers.items()
    }
...
def render_definition(
    ...
    branch_kinds: dict[int, str] = {}
    for cell in definition.cells:
        if cell.token in INLINE_CASES and cell.operand is not None:
            branch_kinds[cell.operand] = "END"
        elif cell.token in BRANCH | CONDITIONAL_BRANCH and cell.operand is not None:
            kind = "R" if cell.operand < cell.address else "C"
            branch_kinds.setdefault(cell.operand, kind)
    kind_counts: dict[str, int] = {}
    for target in sorted(labels):
        kind = branch_kinds[target]
        kind_counts[kind] = kind_counts.get(kind, 0) + 1
        suffix = "" if list(branch_kinds.values()).count(kind) == 1 else f"_{kind_counts[kind]}"
        branch_names[target] = f"{name}_{kind}{suffix}"

    def symbol(value: int, branch: bool = False) -> str:
        if branch and value in labels:
            return branch_names[value]
    ...
    for cell in definition.cells:
        if cell.address in labels:
            lines.append(f"{branch_names[cell.address]}:")
...
def render_action_list(family: Family, local_names: dict[int, str]) -> str:
    lines = [
        f"{family_prefix(family.name)}_ACTIONS:",
...
def rewrite_initializer_block(
    lines: list[str], initializer: Initializer, symbol: str, name: str, space: AddressSpace
) -> list[str]:
    old_label = f"QUESTION_INITIALIZER_{initializer.address:04X}:"
    try:
        start = lines.index(old_label)
    except ValueError as error:
        try:
            start = lines.index(f"{name}:")
        except ValueError:
            raise ValueError(f"missing source initializer at ${initializer.address:04X}") from error
    ...
    while end < len(lines) and consumed < required:
        ...
        if width == 0:
            raise ValueError(f"{name}: cannot measure source line {end + 1}")
        consumed += width
        end += 1
    if consumed < required:
        raise ValueError(f"{name}: source ends before initializer")

    replacement = render_initializer(initializer, symbol, name).splitlines()
    ...
    if surplus:
        ...
        if any(value is None for value in suffix):
            raise ValueError(f"{name}: suffix leaves question bank")
...
def replace_symbols(text: str, replacements: dict[str, str]) -> str:
...
def align_source_comments(lines: list[str]) -> list[str]:
    aligned: list[str] = []
    for line in lines:
        if ";" not in line or not re.match(r"\s+(?:db|dw|rst)\s+", line, re.IGNORECASE):
            aligned.append(line)
            continue
        statement, comment = line.split(";", 1)
        statement = statement.rstrip()
        padding = " " * max(1, 80 - len(statement))
        aligned.append(f"{statement}{padding};{comment}")
    return aligned


def dedupe_equates(lines: list[str]) -> list[str]:
    seen: set[tuple[str, str]] = set()
    result: list[str] = []
    for line in lines:
        match = re.match(
            r"^([A-Za-z_][A-Za-z0-9_]*)\s+equ\s+(\$[0-9A-Fa-f]+)\s*$", line
        )
        if match:
...
def rename_source_symbols(
    lines: list[str], bank, bank_families: list[Family]
) -> list[str]:
    local_names = definition_names(bank_families[0].bank, bank_families, lines)
    initializers = initializer_names(bank_families)
    buckets = bucket_names(bank)
    actual_labels = labels_by_emitted_address(lines)
    replacements: dict[str, str] = {}

    actual_definitions = existing_definition_labels(lines)
    for address, new_name in local_names.items():
        old_name = actual_definitions.get(address, f"PPQ{bank_families[0].bank}_TERSE_{address:04X}")
        replacements[old_name] = new_name

    for address, new_name in initializers.items():
        replacements[f"QUESTION_INITIALIZER_{address:04X}"] = new_name
        for old_name in actual_labels.get(address, ()):
            replacements[old_name] = new_name
    for address, new_name in buckets.items():
        replacements[f"QUESTION_BUCKET_{address:04X}"] = new_name
        for old_name in actual_labels.get(address, ()):
            replacements[old_name] = new_name
    for family in bank_families:
        prefix = family_prefix(family.name)
        replacements[f"PPQ{family.bank}_{family.name}_ACTION_LIST"] = family_symbol(family)
        replacements[f"{family.name}_ACTIONS"] = f"{prefix}_ACTIONS"
        for old_name in actual_labels.get(family.action_list, ()):
            replacements[old_name] = f"{prefix}_ACTIONS"

    # Branch targets are local to one colon definition. Their semantic name
    # records whether control loops, continues forward, or exits a CASES table.
    definitions = {
        definition.address: definition
        for family in bank_families
        for definition in family.definitions
    }
    token_names, operand_names = source_equates(Path(__file__).resolve().parents[1])
    for address, definition in definitions.items():
        rendered = render_definition(
            definition, local_names[address], token_names, operand_names, local_names
        )
        for new_label in re.findall(r"^([A-Za-z_][A-Za-z0-9_]*):$", rendered, re.MULTILINE)[1:]:
            match = re.search(fr"^{re.escape(new_label)}:\n\s+dw.*; \$([0-9A-Fa-f]{{4}})", rendered, re.MULTILINE)
            if match:
                target = int(match.group(1), 16)
                for old_label in actual_labels.get(target, (f"L{target:04X}",)):
                    if target not in definitions:
                        replacements[old_label] = new_label

    text = replace_symbols("\n".join(lines), replacements)
    text = re.sub(r"(; \$[0-9A-Fa-f]{4}) setup$", r"\1 setup action", text, flags=re.MULTILINE)
    text = re.sub(r"(; \$[0-9A-Fa-f]{4}) correct$", r"\1 correct answer", text, flags=re.MULTILINE)
    text = re.sub(r"(; \$[0-9A-Fa-f]{4}) wrong 1$", r"\1 second answer", text, flags=re.MULTILINE)
    text = re.sub(r"(; \$[0-9A-Fa-f]{4}) wrong 2$", r"\1 remaining answer", text, flags=re.MULTILINE)
    return dedupe_equates(align_source_comments(text.splitlines()))
...
def rewrite_sources(root: Path, archive_path: Path, families: list[Family]) -> None:
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        for bank, bank_families in sorted(by_bank.items()):
            ...
            bank_directory = decode_bank(f"ppq{bank}", archive.read(f"ppq{bank}"))
            lines = rename_source_symbols(lines, bank_directory, bank_families)
            local_names = definition_names(bank, bank_families, lines)
            initializer_name_map = initializer_names(bank_families)
            ...
            for initializer, symbol in reversed(work):
                lines = rewrite_initializer_block(
                    lines, initializer, symbol, initializer_name_map[initializer.address], space
                )
...
def check_sources(root: Path, families: list[Family]) -> None:
    ...
    for bank, bank_families in sorted(by_bank.items()):
        ...
        legacy = sorted(set(re.findall(
            r"\b(?:(?:PPQ\d+_TERSE|QUESTION_INITIALIZER|QUESTION_BUCKET)_[0-9A-F]{4}|L[0-9A-F]{4}|[A-Z0-9_]+_WRONG_[12])\b",
            text,
        )))
        if legacy:
            raise ValueError(
                f"src/ppq{bank}.asm retains legacy address labels: "
                + ", ".join(legacy[:5])
            )
        word_addresses = {
            int(match.group(1), 16)
        ...
        for family in bank_families:
            for initializer in family.initializers:
                expected = render_initializer(
                    initializer,
                    family_symbol(family),
                    initializer_name_map[initializer.address],
                )
            ...
            for definition in family.definitions:
                ...
                expected_name = local_names[definition.address]
                if f"{expected_name}:" not in text:
                    raise ValueError(
                        f"src/ppq{bank}.asm:${definition.address:04X} lacks semantic label "
                        f"{expected_name}"
                    )
                missing = {
...
def main() -> None:
    ...
    parser.add_argument(
        "--rewrite-sources",
        action="store_true",
        help="promote graphs and apply canonical semantic PPQ labels",
    )
    parser.add_argument(
        "--check-sources",
        action="store_true",
