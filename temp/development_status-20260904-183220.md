...
1. Apply the proven `ppq1` vocabulary across all 130 bank-local rooted initializer
   families and classify their prompt, answer, timing, animation, graphics,
   and procedural-variant structures.
2. Extend semantic naming from the closed startup/service/banking surface into the
   gameplay, attract, scoring, question-presentation, and initials call graphs.
3. Recover the ten PLS153A fuse maps as Boolean equations and reconcile them
   with the schematics and MAME driver.
4. Classify graphics, sound tables, operator settings, statistics, and
