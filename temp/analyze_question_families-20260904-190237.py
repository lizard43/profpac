...
def decode_action_graph(
...
) -> tuple[frozenset[int], tuple[tuple[int, str], ...]]:
    ...
    return frozenset(cells.values()), prompts
...
def decode_families(archive_path: Path) -> list[Family]:
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        for bank_number in range(1, PPQ_COUNT + 1):
            ...
            for action_list, initializers in sorted(grouped.items()):
                ...
                tokens, prompts = decode_action_graph(space, actions)
    ...
    required = {0x8B4F, 0x8B7B, 0x8C12, 0x8C37, 0x8C50, 0x2302, 0x15F7}
...
def render_report(families: list[Family]) -> str:
    ...
    lines.extend(f"| {label} | {count}/45 |" for label, count in renderer_counts.items())
    lines.extend(
        [
            "",
