...
- `src/ppq1.asm`-`src/ppq14.asm` - byte-exact source for the fourteen question
  ROMs, including all 45 family action lists and their 401 reachable TERSE words
...
- `tools/analyze_question_families.py` - decodes, promotes, and validates family
  identities, complete action graphs, prompts, features, and symbolic source
- `tools/analyze_question_round.py` - validates the fixed question-round
  controller, scheduler closure, bank contexts, and counted action lists
...
- `question_presentation.md` - shared `pps3` question-presentation vocabulary,
  slot allocator, object-state contract, and action lifecycle
- `question_round.md` - complete response controller, scheduler tasks,
  correctness, accounting, bonus, continuation, and post-round transition
...
The build then validates both executable analysis surfaces: 45 PPQ family
graphs and the 130-definition fixed question-round closure.
...
points and 45 distinct action-list families. Their complete transitive closure
contains 401 bank-local colon definitions and 10,532 TERSE cells, all emitted
as structured source. Every family uses a four-action
