...
post-round pointer at `$C002`. Its `$BCDB` action is therefore
`POST_ROUND_TRANSITION_TASK` in `pps8`, not bytes at the same CPU address in
`pps4`.
...
| Exit | Restore configuration 1, copy the configured sound flag into live state, start `POST_ROUND_ACTIONS`, and restore the interrupt/display environment | `POST_ROUND_TRANSITION_TASK` performs the application-level handoff |
...
| `POST_ROUND_ACTIONS` | 1 | configuration-1 `POST_ROUND_TRANSITION_TASK` |

The fixed dispatch directory also contains `ATTRACT_OPEN_ACTIONS`,
`ATTRACT_DEMO_ACTIONS`, `ATTRACT_TITLE_ACTIONS`, `ROUND_RESULT_ACTIONS`, and
`ATTRACT_CLOSE_ACTIONS`. They share the same list representation and are
