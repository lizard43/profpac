...
1. Decode one complete `ppq1` presentation family from rooted initializer
   through action list, action bodies, text, graphics, input, evaluation, and
   cleanup. Use that closed path to establish the PPQ action vocabulary.
2. Apply the proven vocabulary across all 130 bank-local rooted initializer
   families and classify their prompt, answer, timing, animation, graphics,
   and procedural-variant structures.
3. Extend semantic naming from the closed startup/service/banking surface into the
   gameplay, attract, scoring, question-presentation, and initials call graphs.
4. Recover the ten PLS153A fuse maps as Boolean equations and reconcile them
   with the schematics and MAME driver.
5. Classify graphics, sound tables, operator settings, statistics, and
